...
type MessageRepository interface {
	...
	GetMessagesFromChat(ctx context.Context, chatID int64, limit, offset int) ([]models.Message, error)
}
...
func (mr *messageRepository) CreateMessage(ctx context.Context, message *models.Message) error {
	...
	query := `INSERT INTO messages (chat_id, sender_id, image_path, content) VALUES ($1, $2, $3, $4) RETURNING message_id`
	if err := tx.QueryRowContext(ctx, query, message.ChatID, message.SenderID, message.ImagePath, message.Content).Scan(&message.MessageID); err != nil {
		return err
	}

	_, err = tx.ExecContext(ctx, `UPDATE chats SET last_message_at = NOW() WHERE chat_id = $1`, message.ChatID)
	if err != nil {
		return err
	}
	return tx.Commit()
...
func (mr *messageRepository) GetMessagesFromChat(ctx context.Context, chatID int64, limit, offset int) ([]models.Message, error) {
	query := `SELECT * FROM messages WHERE chat_id = $1 LIMIT $2 OFFSET $3 ORDER BY created_at ASC`
	var messages []models.Message
	if err := mr.db.SelectContext(ctx, &messages, query, chatID, limit, offset); err != nil {
		return nil, err
	}
	return messages, nil
}
